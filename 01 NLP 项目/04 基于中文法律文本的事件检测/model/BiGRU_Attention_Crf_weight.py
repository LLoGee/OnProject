...
class BiGRU_Attention_Crf_weight(nn.Module):
    def __init__(self, config):
        super(BiGRU_Attention_Crf_weight, self).__init__()
        self.config = config
        self.embedding = embedding.Embedding(config)
        self.multihead_attention = nn.MultiheadAttention(embed_dim=self.config.getint("model", "hidden_size"), num_heads=1)
    ...
    def forward(self, data, **params):
        ...
        prediction, _ = self.multihead_attention(prediction, prediction, prediction)
