...
class BiGRU_EmbeddingAttention_Crf(nn.Module):
    def __init__(self, config):
        super(BiGRU_EmbeddingAttention_Crf, self).__init__()
        self.config = config
        self.embedding = embedding.Embedding(config)
        self.multihead_attention = nn.MultiheadAttention(embed_dim=self.config.getint("runtime", "embedding_size"), num_heads=4)
