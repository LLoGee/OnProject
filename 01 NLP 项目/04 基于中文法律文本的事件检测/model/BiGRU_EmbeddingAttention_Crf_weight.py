...
class BiGRU_EmbeddingAttention_Crf_weight(nn.Module):
    def __init__(self, config):
        super(BiGRU_EmbeddingAttention_Crf_weight, self).__init__()
        self.config = config
        self.embedding = embedding.Embedding(config)
        self.multihead_attention = nn.MultiheadAttention(embed_dim=self.config.getint("runtime", "embedding_size"), num_heads=1)
    ...
    def forward(self, data, **params):
        ...
        prediction = self.dropout(embedded)

        prediction = self.rnn(prediction, lengths.cpu())  # [B, L, H]
        prediction, _ = self.multihead_attention(embedded, prediction, prediction)
